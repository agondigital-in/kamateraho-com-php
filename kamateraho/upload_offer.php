...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!empty($category_id) && !empty($title) && !empty($description) && !empty($photo)) {
        // Insert offer into database
        $stmt = $conn->prepare("INSERT INTO offers (category_id, title, description, photo) VALUES (?, ?, ?, ?)");
        $stmt->bind_param("isss", $category_id, $title, $description, $photo);
        
        if ($stmt->execute()) {
            $success = "Offer uploaded successfully!";
        } else {
            $error = "Error uploading offer: " . $conn->error;
        }
        $stmt->close();
    } else {
...
<head>
    ...
    <title>Upload Offer - KamateRaho Admin</title>
...
<body>
    ...
    <div class="container">
        <div class="page-header">
            <h1>Upload New Offer</h1>
            <p>Fill in the details below to upload a new offer</p>
        </div>
